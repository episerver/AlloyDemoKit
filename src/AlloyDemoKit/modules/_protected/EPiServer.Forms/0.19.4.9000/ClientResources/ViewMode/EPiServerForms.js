...
(function ($) {
    ...
    $.extend(true, epi.EPiServer.Forms, {
        Utils: {
            ...
            getPreviousPostedFiles: function ($element) {
                // summary:
                ...
                var elementName = $element.attr("name") || $element.data("epiforms-element-name"),
                    formData = epi.EPiServer.Forms.Data.loadCurrentFormDataFromStorage(this.getWorkingFormFromInnerElement($element)),
                    elementValue = $element[0].files;   // the file upload's current chosen files
    ...
    function validateRegularExpressionValidator(fieldName, fieldValue, validatorMetaData) {
        ...
        if (!isMatchedReg(fieldValue, validatorMetaData.model.jsPattern)) {
            ...
            var message = _utilsSvc.stringFormat(validatorMetaData.message, [validatorMetaData.model.jsPattern, validatorMetaData.description]);
            ...
            "EPiServer.Forms.Implementation.Validation.RequiredValidator": function (fielName, fieldValue, validatorMetaData) {
                ...
                if (validatorMetaData) {
                    if (fieldValue == ""
                        ...
                        return { isValid: false, message: validatorMetaData.message };
                    }
                }
            ...
            "EPiServer.Forms.Implementation.Validation.AllowedExtensionsValidator": function (fieldName, fieldValue, validatorMetaData) {
                ...
                for (; j < totalFiles; j++) {
                    if (!__isFileNameValid(files[j].name, unacceptTypes, acceptTypes)) {
                        return {
                            isValid: false,
                            // TODO: should get message from validator's model
                            message: acceptTypes.length === 0
                                ? validatorMetaData.model.message
                                : epi.EPiServer.Forms.Utils.stringFormat(validatorMetaData.model.messageWithParams, [acceptValue])
                        }
                    }
                }
            ...
            "EPiServer.Forms.Implementation.Validation.MaxFileSizeValidator": function (fieldName, fieldValue, validatorMetaData) {
                ...
                for (; i < totalItems; i++) {
                    if (files[i].size > expectedSize) {
                        return { isValid: false, message: epi.EPiServer.Forms.Utils.stringFormat(validatorMetaData.message, [expectedSize / (1024 * 1024)]) };
                    }
                }
    ...
    epi.EPiServer.Forms.init = function () {
        ...
        $(document).ready(function () {
            ...
            function _formResetHandler(/*Event*/e) {
                ...
                // Navigate to the first step of the working form incase RESET button clicked.
            ...
            function _resetFormValues($workingForm, workingFormInfo) {
                ...
                $(".Form__Element", $workingForm).each(function loopEachFormElementToClear(i, formElement) {
                    ...
                    else if ($el.find(".FormFileUpload__Input").length > 0 || formElement.type === "file" || formElement.files) {  // after refactoring the FileUload, we only need to check $el.hasClass("FormFileUpload")
            ...
            function _doSubmitForm(/*Event*/e) {
                ...
                var data = saveCurrentFormDataToStorage(workingFormInfo.$workingForm),
                ...
                $.ajax({
                    ...
                    success: function onFormSubmittingSuccess(returnedResult) {
                        ...
                        if (returnedResult.IsSuccess === true) {
                            ...
                            var $currentStep = _getCurrentStepDOM(workingFormInfo);
                            $(".FormFileUpload", $currentStep).each(function (index, item) {
                                $(this).val("");
                            ...
                            if (isFinalizedSubmission == false) {
                                ...
                                _storage.setItem(_utilsSvc.getFormIdentifier($workingForm), JSON.stringify(newMergeData));
                            ...
                            if (returnedResult.FailInfo && returnedResult.FailInfo.InvalidElement) {
                                ...
                                epi.EPiServer.Forms.Validators[validator.type]["onServerValidateFailed"]
                                    && epi.EPiServer.Forms.Validators[validator.type]["onServerValidateFailed"]($element, validator);

                                $messageContainer.text(epi.EPiServer.Forms.Messages.viewMode.commonValidationFail).show();
                            }
                        }
                    },
            ...
            function _rebindElementWithFormStorageData(formData, $workingForm) {
                ...
                $.each(formData, function (name, val) {
                    ...
                    if ($item.length > 0) { //found the input control that should hold field value
                        ...
                        if ($item.hasClass("FormFileUpload")) {
            ...
            function _getPostedFileMessage($fileUploadElement) {
                // TODO: we will not have to nagivate to parent() if $fileUploadElement is the Form__Element root DOM of FileUpload (after future refactoring)
                var $messageContainer = $fileUploadElement.find(".FormFileUpload__PostedFile");
                if ($messageContainer.length <= 0) {
                    $messageContainer = $fileUploadElement.parent().find(".FormFileUpload__PostedFile");
                }
            ...
            // Try to save what data Visitor curently enterred into localStorage. 
            // New data will be merged into old data, and new field's value will override old one.
            // @return newMergeData is save successfully
            function saveCurrentFormDataToStorage($workingForm, excludeCallback) {
                var newMergeData = getCurrentStepData($workingForm, excludeCallback);
            ...
            function collectInputData($workingForm, excludeCallback) {
                ...
                // Get fileupload data
                $(".FormFileUpload", $workingForm).each(function (index) {
                    ...
                    if (this.files.length === 0) {
                        var files = _utilsSvc.getPreviousPostedFiles($(this));
            ...
            function navigationInit(workingFormInfo) {
                ...
                $(".Form__NavigationBar__Action.btnPrev", workingFormInfo.$workingForm).on("click", function (e) {
                    ...
                    saveCurrentFormDataToStorage(workingFormInfo.$workingForm, /*excludeCallback*/function __excludeFileUpload($element) {
                        return $element.hasClass("FormFileUpload");
                    }); // save form state before changing step
